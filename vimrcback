...
"set smartindent
set lbr "不在单词中间断行
set fo+=mB "Ｍ表示允许在两个汉字之间断行，即使汉字间没有出现空格，Ｂ表示把两行合并为一行的时候，汉字与汉字间不要补空格
set sm "显示括号配对情况,打开这个配置后，会显示括号配对情况
"set cin 打开Ｃ，Ｃ＋＋的风格缩进　set cino=:0g0t0(sus
"set keymodel=startsel,stoipsel 用shift + 方向键选择文本，方向键代表选择哪一个单词
set selection=inclusive "指定在选择文本时，光标所在位也属于选择位置。另一选项exclusive
set wildmenu
set mousemodel=popup "右键单击窗口时弹出快捷菜单
"set splitbelow
"set splitright
